...
import com.bber.customview.listener.OnTagClickListener;
...
public class FlowTagLayout extends ViewGroup {
     ...
     * 标签流式布局,最大选中个数
     */
    private int mTagCheckMax = 0;
    /**
     * 每行显示标签的数量
     */
    private int mTagLineSum = 0;
     ...
     * 重新加载刷新数据
     */
    public void setmCheckedTagArray(int selectIndex){
        mCheckedTagArray.put(selectIndex, true);
        mChildView.get(selectIndex).setSelected(true);
    }
    /**
     * 重新加载刷新数据
     */
            ...
            childView.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    if (mTagCheckMode == FLOW_TAG_CHECKED_NONE) {
                        ...
                        if (mTagCheckMax > 0){
                            int mTagChecked = 0;
                            for (int k = 0; k < mCheckedTagArray.size(); k++) {
                                if (mCheckedTagArray.get(k)) {
                                    mTagChecked++;
                                }
                            }

                            if (mTagChecked >= mTagCheckMax
                                    && !mCheckedTagArray.get(j)){
                                return;
                            }
                        }
     ...
     * @param TagCheckMax
     */
    public void setTagCheckedMax(int TagCheckMax) {
        this.mTagCheckMax = TagCheckMax;
    }
    /**
     * 设置标签每行的个数
     *
     * @param TagLineSum
     */
    public void setTagLineSum(int TagLineSum) {
        this.mTagLineSum = TagLineSum;
    }
